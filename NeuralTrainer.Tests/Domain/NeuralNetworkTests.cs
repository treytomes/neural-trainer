...
{
	public class NeuralNetworkLearningRateTests
	{
		[Theory]
		[InlineData(0)]
		[InlineData(-0.001)]
		...
		{
			// Act & Assert
			var exception = Assert.Throws<ArgumentOutOfRangeException>(() => new NeuralNetwork(invalidLearningRate));
		...
		{
			// Act
			var network = new NeuralNetwork(validLearningRate);
		...
		[Fact]
		public void Constructor_ShouldUseDefaultLearningRate_WhenNoValueProvided()
		{
			// Act
			...
			// Assert
			Assert.NotNull(network);
			// The default of 0.1 should not throw any exceptions
		}
		...
		public void Constructor_ShouldThrowArgumentException_WhenLearningRateIsNotFinite(double invalidLearningRate)
		{
			// Act & Assert
			var exception = Assert.Throws<ArgumentOutOfRangeException>(() => new NeuralNetwork(invalidLearningRate));
			// The exact message might vary based on your implementation
		...
		{
			// Arrange
			var network = new NeuralNetwork(learningRate);
		...
		{
			// Arrange
			var slowNetwork = new NeuralNetwork(0.1);
			var fastNetwork = new NeuralNetwork(0.5);
			var trainingData = new[]
			{
				new TrainingExample(0, 1),
				new TrainingExample(1, 0)
