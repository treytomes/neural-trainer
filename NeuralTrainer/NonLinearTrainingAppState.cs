...
class NonLinearTrainingAppState(IActivationFunction activationFunction, IWeightInitializer weightInitializer, ILossFunction lossFunction, IProgressReporter progressReporter) : IAppState
{
	#region Methods
	...
	private INeuralNetwork TrainGate(string gateName, IEnumerable<TrainingExample> trainingData)
	{
		...
		var network = new NeuralNetwork([2, 4, 3, 1], activationFunction, weightInitializer);
		...
		trainer.Train(network, trainingData, epochs: 100000);
